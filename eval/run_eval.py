...
async def measure_ingestion_throughput(data_dir: Path, runs: int = 1) -> float:
    """Measure docs/sec for full corpus ingestion."""
    print(f"\n=== Ingestion Throughput ({runs} run) ===")
    ...
    for i in range(runs):
        ...
        manifest = await ingest_corpus(data_dir=data_dir, vector_store=vector_store, manifest_store=manifest_store)
...
def measure_verification_latency(cert_count: int = 50) -> float:
...
def measure_tamper_detection() -> tuple[float, float]:
...
async def main():
    ...
    try:
        metrics['ingest_throughput'] = await measure_ingestion_throughput(data_dir, runs=1)
    ...
    try:
        manifest_store = ManifestStore()
        metrics['proof_size_kb'] = await measure_proof_size(manifest_store)
    except Exception as e:
    ...
    try:
        tamper_rate, false_positive_rate = measure_tamper_detection()
